...
import { StoryViewerProps } from '@/types/stories.types'
import { markProductAsViewed } from '@/lib/stories-progress-utils'
...
const StoryViewer = ({
  ...
  const progressIncrement = (100 / autoAdvanceInterval) * 100 // 100 = interval tick (100ms)

  // Navigate to next product/store
  const goToNext = useCallback(() => {
    // Mark current product as viewed before advancing
  ...
  }, [currentProductIndex, totalProducts, currentStoreIndex, stores.length, onClose, currentProduct, currentStore])
  ...
  const handleActionClick = () => {
    if (onStoryActionClick && currentProduct && currentStore) {
      onStoryActionClick(currentProduct, currentStore)
    }
  }
  ...
  }, [goToNext, progressIncrement])

  // Mark current product as viewed when displayed
  useEffect(() => {
    if (currentProduct && currentStore) {
      markProductAsViewed(
        currentStore.id,
  ...
  }, [currentProduct?.id, currentStore?.id, currentStore?.products.length])
  ...
  useEffect(() => {
    ...
    stickyFilters.forEach((el) => ((el as HTMLElement).style.display = 'none'))

    return () => {
      document.body.style.overflow = ''
      if (mobileNav) (mobileNav as HTMLElement).style.display = ''
      stickyFilters.forEach((el) => ((el as HTMLElement).style.display = ''))
    }
  }, [])
    ...
    <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-[380px] mx-4 h-[90vh] max-h-[750px] rounded-3xl overflow-hidden shadow-2xl bg-black">
        ...
        <div className="absolute inset-0">
          <img
            src={currentProduct.imageUrl || currentStore.imageUrl}
        ...
        <div className="absolute inset-0 flex z-10">
          ...
          />
          <div className="w-1/3" />
          <button className="w-1/3 h-full cursor-pointer" onClick={goToNext} aria-label="Next story" />
        </div>
        ...
        <div className="absolute bottom-4 left-3 right-3 z-20">
          <div className="bg-white rounded-2xl p-4 shadow-xl">
            ...
            <div className="flex items-center justify-between mb-2">
                ...
                {currentProduct.badges?.map((badge, idx) => (
                  <span
                    ...
                    className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                      badge.type === 'hot' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'
                    }`}
                  >
                    {badge.label}
                  </span>
              ...
              <span className="text-muted-foreground text-[11px]">Sold by {currentStore.name}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="text-green-600 font-bold text-lg">{currentProduct.price}</span>
          ...
          {stores.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all ${
                index === currentStoreIndex ? 'w-5 bg-white' : 'bg-white/40 hover:bg-white/60'
              }`}
